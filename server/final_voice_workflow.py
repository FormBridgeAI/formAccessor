"""
Final Voice-Enabled Form Filling Workflow with TTS and Data Formatting
Combines Azure Document Intelligence, OpenAI, Deepgram Speech-to-Text, and Text-to-Speech
Includes proper data formatting for dates, addresses, phone numbers, etc.
"""
...
from voice_form_filler_with_formatting import VoiceFormFillerWithFormatting

# Load environment variables
load_dotenv("../.env")

def final_voice_form_workflow(input_image_path: str, use_voice: bool = True):
    """
    Final voice-enabled form filling workflow with TTS and data formatting
    ...
    print("🚀 Starting Final Voice-Enabled Form Workflow...")
    ...
    if use_voice:
        print("\n🎤 Step 3: Voice-enabled form filling with TTS and formatting...")
        print("Starting voice form filler with text-to-speech and data formatting...")
        
        # Create voice form filler with TTS and formatting
        voice_filler = VoiceFormFillerWithFormatting(schema)
...
def main():
    """Main function to run the final voice-enabled workflow"""
    ...
    print("1. Voice interaction with TTS and formatting (AI speaks questions, you speak answers)")
    ...
    # Run the workflow
    result = final_voice_form_workflow(input_image, use_voice)
    
    if result:
        print("\n🎉 Final voice-enabled workflow completed successfully!")
